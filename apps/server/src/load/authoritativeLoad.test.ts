...
const OUTLIER_TICK_LIMIT_MS = 250;
...
describe('charge longue du serveur autoritaire', () => {
  it('simule vingt minutes, quatre joueurs et deux cents monstres dans les budgets', () => {
    ...
    const p99TickMs = percentile(durations, 0.99);
    const p95PatchBytes = percentile(patchSizes, 0.95);
    console.info(
      `[server-load] ${TICKS_IN_TWENTY_MINUTES} ticks · tick p95 ${p95TickMs.toFixed(3)} ms · p99 ${p99TickMs.toFixed(3)} ms · patch p95 ${String(Math.round(p95PatchBytes / 1024))} Kio · ferraille max ${maxScraps}`,
    );
    expect(runtime.snapshot().tick).toBe(TICKS_IN_TWENTY_MINUTES);
    expect(p95TickMs).toBeLessThan(P95_TICK_BUDGET_MS);
    expect(p99TickMs).toBeLessThan(P99_TICK_BUDGET_MS);
    expect(Math.max(...durations)).toBeLessThan(OUTLIER_TICK_LIMIT_MS);
